...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = models.__dict__[args.arch](num_classes=num_classes, use_norm=use_norm, num_trans=4)
...
def train(train_loader, model, criterion, optimizer, epoch, args, log, tf_writer, CE):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        rot0 = input
        rot90 = torch.rot90(input, 1, [2, 3])
        rot180 = torch.rot90(input, 2, [2, 3])
        rot270 = torch.rot90(input, 3, [2, 3])
        rot0_label = torch.zeros(input.size(0), 1)
        rot90_label = rot0_label + 1
        rot180_label = rot0_label + 2
        rot270_label = rot0_label + 3

        rotclass_label = torch.stack([target, target, target, target], dim=0)
        rot_data_cat = torch.stack((rot0, rot90, rot180, rot270), dim=0)
        rot_label_cat = torch.stack((rot0_label, rot90_label, rot180_label, rot270_label), dim=0)
        rot_data_cat = torch.transpose(rot_data_cat, 0, 1)
        rot_label_cat = torch.transpose(rot_label_cat, 0, 1)
        rotclass_label = torch.transpose(rotclass_label, 0, 1)

        idx = torch.randint(4, size=(rot_data_cat.size(0),))
        rotdata = rot_data_cat[torch.arange(rot_data_cat.size(0)), idx]
        rotlabel = rot_label_cat[torch.arange(rot_label_cat.size(0)), idx].reshape(-1).long().cuda()
        rotclass_label = rotclass_label[torch.arange(rotclass_label.size(0)), idx].reshape(-1).long().cuda()

        # compute output
        output = model(input)
        loss = criterion(output, target)

        rotoutput = model(rotdata, rot=True)
